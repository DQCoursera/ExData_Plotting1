...
png("plot4.png", height=480,width=480) #initial plot creation

par(mfrow=c(2,2))#create 2x2 grid for the plots to be on

#the top left plot is plot2.r with minor difference in labeling
...
#top right plot is similar to top left but x axis label and voltage as y axis

# Create the plot
plot(as.POSIXct(paste(febdate$Date, febdate$Time), format="%Y-%m-%d %H:%M:%S"),
     febdate$Voltage, type="l",
     ylab="Voltage", xlab="",
     xaxt = "n")  # remove default tick labels

# Customize x-axis labels to show only 3 points (Thu, Fri, Sat)
#labels have already been created in lines 43-54 above
axis(1, at = tick_positions,             # Positions where we want the labels
     labels = c("Thu", "Fri", "Sat"))    # Labels corresponding to these positions

mtext("datetime", side=1, line=2) # adding 'datetime' underneath other x axis labels


#lower left plot is identical to plot3.R
# Create the plot
plot(as.POSIXct(paste(febdate$Date, febdate$Time), format="%Y-%m-%d %H:%M:%S"),
     febdate$Sub_metering_1,
     type="l",
     ylab="Energy sub metering", xlab="",
     xaxt = "n")  # remove default tick labels

lines(as.POSIXct(paste(febdate$Date, febdate$Time), format="%Y-%m-%d %H:%M:%S"), 
      febdate$Sub_metering_2, col="red") #adding the other 2 lines
lines(as.POSIXct(paste(febdate$Date, febdate$Time), format="%Y-%m-%d %H:%M:%S"), 
      febdate$Sub_metering_3, col="blue") #same x value as initial plot
...
legend("topright", legend=c("sub_metering_1","sub_metering_2","sub_metering_3"),col=c("black","red","blue"),lty=1) #add legend

#lower right plot is similar to upper right plot but global_reactive_power instead of voltage

plot(as.POSIXct(paste(febdate$Date, febdate$Time), format="%Y-%m-%d %H:%M:%S"),
     febdate$Global_reactive_power, type="l",
     ylab="Global_reactive_power", xlab="",
     xaxt = "n")  # remove default tick labels

# Customize x-axis labels to show only 3 points (Thu, Fri, Sat)
#labels have already been created in lines 13-24 above
axis(1, at = tick_positions,             # Positions where we want the labels
     labels = c("Thu", "Fri", "Sat"))    # Labels corresponding to these positions
